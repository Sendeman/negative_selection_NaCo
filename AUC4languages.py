...
def gen_AUC(r: int, lang, axis=None):
    samples = []
    with open(f'task3/english_train_english_test_r_{r}', mode='r') as f:
        for line in f:
            samples.append((float(line.rstrip()), int(0)))
    with open(f'task3/english_train_{lang}_test_r_{r}', mode='r') as f:
        for line in f:
    ...
    axis.set(xlabel='1 − specificity', ylabel='sensitivity', title=f"negsel2.jar ROC curve for {lang} (r={r}) | AUC={round(auc, 2)}")
...
langs = ["xhosa","plautdietsch","hiligaynon","middle-english"]
r=3
pos = [(0,0), (0,1), (1,0), (1,1)]
for i, lang in enumerate(langs):
    gen_AUC(r=r, lang=lang, axis=axs[pos[i][0], pos[i][1]])    
