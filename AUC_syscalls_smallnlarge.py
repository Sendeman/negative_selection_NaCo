...
def gen_AUC(rs: tuple, n: int, size: str, axis=None, pref="cert"):
    assert size=="small" or size=="large", "size need to be one of \"small\" or \"large\""
    max_auc=0
    max_r=None
    for r in rs:
        res = f"syscalls/negselresults/{size}/{pref}_n_{n}_r_{r}"
        labels = f"syscalls/snd-{pref}/{size}/{size}.labelss"
...
ns=[]
rs=[]
for n in range(3,8): #Populate n parameters and r parameters same way as bash script
    ns.append(n)
    rtemp=[]
...
for i, n in enumerate(ns):
    gen_AUC(rs=rs[i], n=n, size=size, axis=axs[pos[i][0], pos[i][1]])    

fig.suptitle(f"Ablation study for {size} syscalls", fontsize=15)
...
for n in [15, 20, 25, 30]: #Populate n parameters and r parameters same way as bash script
    print(n)
    ns.append(n)
    rtemp=[]
